...
async function getExtractionAgentConfig() {
...
export async function startExtraction(text: string, sourceUrl?: string) {
  ...
  const resourceId = sourceUrl ?? `extraction-${crypto.randomUUID()}`;
  extractionProgress.clear(resourceId);

  llmStore.setInputText(text);
  llmStore.setSourceUrl(sourceUrl ?? null);
  llmStore.setExtractionResourceId(resourceId);
  ...
  try {
    ...
    extractionProgress.emit({ type: 'stage-start', resourceId, stage: 'extract', statusText: 'Streaming LLM response...' });
    ...
    const streamResult = await llm.streamExtraction(
      ...
      (chunk) => {
        extractionProgress.emit({ type: 'llm-chunk', resourceId, text: chunk });
      },
      (info) => {
    ...
    extractionProgress.emit({ type: 'stage-complete', resourceId, stage: 'extract', meta: { ms: Date.now() - extractStart } });

    extractionProgress.emit({ type: 'stage-start', resourceId, stage: 'validate', statusText: 'Parsing JSON response...' });
    ...
    extractionProgress.emit({ type: 'stage-complete', resourceId, stage: 'validate', meta: { ms: Date.now() - validateStart }, statusText: `${items.filter(i => i.type === 'node').length} entities, ${items.filter(i => i.type === 'edge').length} relationships` });

    useLLMStore.getState().setDiff({ items, notes: extractedNotes });
    useLLMStore.getState().setStatus('extracted');
  } catch (e: any) {
    extractionProgress.emit({ type: 'error', resourceId, stage: 'extract', message: e.message });
    useLLMStore.getState().setError(e.message);
  }
}

export async function startQuickExtraction(prompt?: string, sourceUrl?: string) {
  // Privacy disclosure gate
  const disc = await storage.get('privacyDisclosureAccepted') as Record<string, any>;
  if (!disc.privacyDisclosureAccepted) {
  ...
  extractionProgress.emit({ type: 'stage-start', resourceId, stage: 'fetch', statusText: sourceUrl ? `Fetching ${sourceUrl}...` : 'Reading page content...' });
    ...
    try {
      ...
      if (fetchResult?.error) {
        if (fetchResult.blocked) {
          extractionProgress.emit({ type: 'error', resourceId, stage: 'fetch', message: fetchResult.error });
      ...
      pageContent = { title: titleMatch?.[1]?.trim() ?? sourceUrl, url: sourceUrl, content };
    } catch (e: any) {
      extractionProgress.emit({ type: 'error', resourceId, stage: 'fetch', message: e.message });
  ...
  extractionProgress.emit({ type: 'stage-complete', resourceId, stage: 'fetch', meta: { bytes: pageContent.content.length, ms: Date.now() - fetchStart } });
  ...
  try {
    ...
    const edgeCount = items.filter(i => i.type === 'edge').length;
    extractionProgress.emit({ type: 'stage-complete', resourceId, stage: 'validate', meta: { ms: Date.now() - validateStart }, statusText: `${nodeCount} entities, ${edgeCount} relationships` });
...
export async function startAgentExtraction(prompt?: string, sourceUrl?: string) {
  // Privacy disclosure gate
  const disc = await storage.get('privacyDisclosureAccepted') as Record<string, any>;
  if (!disc.privacyDisclosureAccepted) {
...
export async function startIngestion(source: IngestionSource, mode: ProcessingMode = 'full') {
  // Privacy disclosure gate
  const disc = await storage.get('privacyDisclosureAccepted') as Record<string, any>;
  if (!disc.privacyDisclosureAccepted) {
...
export async function regenerateExtraction(feedback: string) {
  const llmStore = useLLMStore.getState();
  const inputText = llmStore.inputText;
  const sourceUrl = llmStore.sourceUrl;

  if (!inputText) {
    llmStore.setError('No original content available for regeneration');
    return;
  }

  // Summarize previous extraction so the LLM knows what to change
  const prevDiff = llmStore.diff;
  let prevSummary = '';
  if (prevDiff) {
    const prevNodes = prevDiff.items.filter(i => i.type === 'node').map(i => (i.extracted as any).name);
    const prevEdges = prevDiff.items.filter(i => i.type === 'edge').map(i => {
      const e = i.extracted as any;
      return `${e.sourceName} → ${e.label} → ${e.targetName}`;
    });
    prevSummary = `The previous extraction produced these entities: ${prevNodes.join(', ')}`;
    if (prevEdges.length > 0) prevSummary += `\nRelationships: ${prevEdges.join('; ')}`;
  }

  // Update reading list card if this resource came from the reading list
  const readingListId = sourceUrl?.trim() ?? '';
  const readingListItem = readingListId ? useReadingListStore.getState().items[readingListId] : undefined;
  if (readingListItem) {
    useReadingListStore.setState((state) => ({
      items: { ...state.items, [readingListId]: { ...state.items[readingListId], status: 'processing' as const } },
    }));
  }

  // Reset review state but preserve inputText and sourceUrl
  useExtractionReviewStore.getState().reset();
  llmStore.setError(null);
  llmStore.setDiff(null);

  const resourceId = sourceUrl ?? `extraction-${crypto.randomUUID()}`;
  extractionProgress.clear(resourceId);
  llmStore.setExtractionResourceId(resourceId);
  llmStore.setStatus('extracting');
  useUIStore.getState().focusContentTab('extraction-review');
  ...
  try {
    ...
    const systemPrompt = getQuickExtractSystemPrompt(notesOn, instructions, graphContext);

    let userContent = `IMPORTANT — Re-extract this content differently.`;
    if (prevSummary) userContent += `\n\n${prevSummary}`;
    userContent += `\n\nThe user's feedback on the previous extraction:\n${feedback}`;
    userContent += `\n\n---\n\nExtract entities and relationships from the following content:\n\n${inputText}`;

    // Stage: extract
    extractionProgress.emit({ type: 'stage-start', resourceId, stage: 'extract', statusText: 'Re-extracting with feedback...' });
    const extractStart = Date.now();
    ...
    // Restore reading list item to ready if it was from reading list
    if (readingListItem && readingListId) {
      useReadingListStore.setState((state) => ({
        items: { ...state.items, [readingListId]: { ...state.items[readingListId], status: 'ready' as const } },
      }));
    }
