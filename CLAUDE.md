...
- **Platform abstraction**: UI imports `@platform` — never touches `chrome.*` or `ipcRenderer` directly. Eight interfaces (`PlatformStorage`, `PlatformDB`, `PlatformNotes`, `PlatformLLM`, `PlatformBrowser`, `PlatformEmbedding`, `PlatformVault`, `PlatformArtifacts`). See [`docs/platform-layer.md`](docs/platform-layer.md).
...
- **Artifact system**: LLM-generated content (JSX, Markdown, HTML, SVG, Mermaid) persisted in `.kg/artifacts/`, browsable via side rail, rendered in sandboxed tabs. See [`docs/artifact-system.md`](docs/artifact-system.md).
- **Build system**: Two Vite configs (Chrome/Electron) + esbuild for main process + esbuild sandbox vendor bundle. See [`docs/build-system.md`](docs/build-system.md).
...
- **State management**: Twelve independent Zustand stores in `src/graph/store/` (graph, ui, llm, node-type, extraction-review, agent, artifact).
...
- `src/shared/artifact-types.ts` — `ArtifactType`, `ArtifactMeta`, `ArtifactRecord`, `slugify()`
...
| [`docs/artifact-system.md`](docs/artifact-system.md) | Artifact types, storage layout, sandbox renderer, tool definitions, UI components |
