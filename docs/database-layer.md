...
- **`src/db/worker/migrations/`** — Versioned, FTS5 detected at runtime. Migration 002 (FTS) is optional; search falls back to LIKE. Migration 010 adds `location TEXT` to `entity_sources`/`edge_sources` (JSON-serialized SourceLocation for provenance) and `vault_path TEXT` on `nodes`. Migration 014 (schema cleanup) drops dead tables (`extraction_log`, `note_folders`, `indexed_files`, `memory_semantic`, `memory_episodic`, `embedding_dismissals`, and MCP-only drifted tables `spatial_positions`, `reading_list`, `browsing_history`) and removes stale columns from `nodes`, `edges`, `chat_messages`, `chat_sessions`, `note_attachments`, `ontology_node_types`, `ontology_edge_types`, `source_content`, `reading_list_history`, and `embedding_metadata`. It also repairs vaults that were initialized by the old standalone synapse-mcp CLI and may be missing tables created after their initial schema; those vaults gain all missing tables but remain without FTS (search falls back to LIKE, per [docs/search.md](search.md)). Each migration runs atomically inside `BEGIN IMMEDIATE / COMMIT`; a failure rolls back the entire migration and leaves the schema at its previous version.
